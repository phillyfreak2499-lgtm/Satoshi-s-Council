...
// `.grok/app-env.json` is a per-workspace, gitignored deploy-flag file (see the
// header of with-app-env.mjs). A clean checkout has none, and the wrapper's
// documented default there is auth-on. The three cases below pin the auth-off
// default that file encodes, so they only carry meaning where the file exists —
// skip them, rather than fail, when it does not.
...
test("the template ships auth off", { skip: APP_ENV_SKIP }, () => {
...
test("the wrapped command runs with the app env applied", { skip: APP_ENV_SKIP }, async () => {
...
test("the CLI still runs when invoked through a symlinked path", { skip: APP_ENV_SKIP }, async () => {
