...
def wilson_lower_bound(successes: int, n: int, z: float = 1.96) -> float:
    ...
    Gives a conservative estimate of true win-rate that strongly prefers
    larger samples over lucky short streaks. Used in hierarchy ranking
    so the Chair listens more to proven agents and less to noisy early data.
    This improves confluence quality and therefore call accuracy over time.
    """
    if n <= 0:
        return 0.0
...
class AdaptiveLearner:
    ...
    def hierarchy_ranks(self) -> List[Dict[str, Any]]:
        ...
        Score prioritizes Wilson lower-bound (evidence-preferring) + Laplace
        smoothed WR + prior weight + light correct-count tie-break.
        Wilson strongly prefers agents with larger samples over lucky 3/3s,
        improving Chair listen factors and confluence quality for better calls.
        """
        voters = [
        ...
        for name in voters:
            ...
            wr = (c + 2) / (n + 4)
            wilson = wilson_lower_bound(c, n)
            weight = float(self.weights.get(name, prior_weight(self.asset, name)))
            # Evidence-first: Wilson dominates early ranking quality;
            # Laplace keeps cold-start mid-pack; weight + volume light ties.
            score = (
                wilson * 100.0
                + wr * 30.0
                + weight * 6.0
    ...
    def learn_from_settled(
        ...
        for name, vote in votes.items():
            ...
            # Normalise the scalp engine's language so LONG_* / HOLD seats are
            # graded on direction instead of being silently skipped — they are
            # the bulk of the directional voters (momentum, odds, strike,
            # spotlag…), so dropping them left finish-learning nearly blind.
            if d in ("LONG_UP", "UP_HOLD"):
                d = "UP"
            elif d in ("LONG_DOWN", "DOWN_HOLD"):
                d = "DOWN"
            if d not in ("UP", "DOWN"):
                continue
