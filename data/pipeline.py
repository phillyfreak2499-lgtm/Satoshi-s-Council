...
class DataPipeline:
    def __init__(self, asset: str = "btc", series_ticker: str | None = None, symbol: str | None = None):
        ...
        self._last_good_ttl_s: float = 180.0
    ...
    def _merge_last_good(self, snap: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not prev:
            return snap
        age = time.time() - float(self._last_good_at or 0)
        if age > float(self._last_good_ttl_s):
            return snap
        filled = False
        for k in (
    ...
    async def fetch(self) -> Dict[str, Any]:
        ...
        if snap.get("current_price") is not None and not snap.get("last_good"):
