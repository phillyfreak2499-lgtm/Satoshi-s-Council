...
def to_cents(raw: Any) -> Optional[float]:
    """Kalshi unit quotes are (0, 1). Integer cents stay cents.

    0.47 → 47.  1 → 1.  42 → 42.  A raw 1 is one cent, not one dollar.
    """
    n = _f(raw)
    if n is None:
        return None
...
def enrich_snap(snap: Dict[str, Any]) -> Dict[str, Any]:
    ...
    snap_ticker = str(km.get("ticker") or snap.get("market_ticker") or snap.get("kalshi_ticker") or "").strip()
    cache_ticker = str(row.get("ticker") or "").strip()
    same_book = (not snap_ticker) or (not cache_ticker) or (snap_ticker == cache_ticker)
    if same_book:
