...
from backend.services.self_fonts import ensure_self_fonts
...
def _stitch_fonts(root: Path) -> None:
    ...
    try:
        ...
        return
    if MARKER in text and "orbitron-700.woff2" in text:
        return
    chunks = [MARKER]
    if fonts.is_file():
        chunks.append(fonts.read_text(encoding="utf-8"))
    if rooms.is_file():
        rooms_txt = rooms.read_text(encoding="utf-8")
        if rooms_txt not in text:
            chunks.append(rooms_txt)
    try:
        # Drop a stale marker-only stitch so fonts can land.
