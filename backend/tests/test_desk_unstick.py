...
class HydrateSeatsTests(unittest.TestCase):
    ...
    def test_closer_does_not_own_the_analyze_lock(self) -> None:
        self.assertIn("Do NOT take _analyze_lock", DUAL)
        self.assertIn("LOCK_WAIT_S", DUAL)
        self.assertIn("def _touch", DUAL)
        self.assertIn("loop_heartbeat", DUAL)
        self.assertIn("lock_busy", DUAL)
