...
class WireLogTests(unittest.TestCase):
    def test_comment_and_shape(self):
        ...
        self.assertEqual(rows[0]["id"], "2026-08-16-desk-unlock-stay")
        self.assertIn("After SUMMON the gate stays hidden", rows[0]["why"])
        self.assertIn("new 6s opening plays", rows[0]["why"])
        self.assertIn("old logo intro is gone", rows[0]["why"])
        self.assertIn("Leader photo click selects only", rows[0]["why"])
        ...
        self.assertEqual(rows[1]["id"], "2026-08-15-hit-slate-reset")
        self.assertIn("3,053 hours", rows[1]["why"])
        self.assertIn("old 2/7", rows[1]["why"])
        self.assertIn("Paper only", rows[1]["why"])
        self.assertIn("Follower OFF", rows[1]["why"])
        for row in rows:
