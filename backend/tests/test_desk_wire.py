...
class WireLogTests(unittest.TestCase):
    def test_comment_and_shape(self):
        ...
        self.assertEqual(rows[0]["id"], "2026-08-16-kill-crickets")
        self.assertIn("Cricket", rows[0]["title"])
        self.assertIn("cricket", rows[0]["why"])
        self.assertIn("Night", rows[0]["why"])
        self.assertIn("SFX stay", rows[0]["why"])
