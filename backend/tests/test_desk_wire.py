...
class WireLogTests(unittest.TestCase):
    def test_comment_and_shape(self):
        ...
        self.assertEqual(rows[0]["id"], "2026-08-16-coinglass-hud-only")
        self.assertIn("HUD", rows[0]["title"])
        self.assertIn("not green", rows[0]["why"])
        self.assertIn("does not chase", rows[0]["why"])
        self.assertIn("401 probe path", rows[0]["why"])
