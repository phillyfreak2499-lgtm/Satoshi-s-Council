...
class WireLogTests(unittest.TestCase):
    def test_comment_and_shape(self):
        ...
        self.assertEqual(rows[0]["id"], "2026-08-16-coinglass-1h-only")
        self.assertIn("1h hist", rows[0]["why"])
        self.assertIn("4h", rows[0]["why"])
        self.assertIn("exchange-list", rows[0]["why"])
        self.assertIn("PR #31", rows[0]["why"])
