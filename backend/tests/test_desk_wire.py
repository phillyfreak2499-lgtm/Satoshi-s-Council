...
class WireLogTests(unittest.TestCase):
    def test_comment_and_shape(self):
        ...
        self.assertEqual(rows[0]["id"], "2026-08-15-hit-slate-reset")
        self.assertIn("3,053 hours", rows[0]["why"])
        self.assertIn("old 2/7", rows[0]["why"])
        self.assertIn("Paper only", rows[0]["why"])
