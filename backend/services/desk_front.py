...
Weather chair: RAIJIN. Same weight as Satoshi / Vitalik on this tab.
No invented portrait. Fillable slot or neon thunder-god mark. Not gold ZT.

Four WX seats only: FORECAST, MARKET, SKIP, CLIMO.
...
CHAIR: Dict[str, Any] = {
    "id": "RAIJIN",
    "name": "RAIJIN",
    "job": "Weather chair. Same weight as Satoshi / Vitalik on THE FRONT. Does not lock the 1H Chair.",
    "slot": "/static/bots/raijin.png",
}
...
def build_chair(best: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    call = None
    if best:
        call = best.get("skip") or f"{best.get('city') or ''} {best.get('bracket') or ''}".strip()
    return {
        "id": CHAIR["id"],
        "name": CHAIR["name"],
        "job": CHAIR["job"],
        ...
        "city": None if not best else best.get("city"),
