...
class DualOrchestrator:
    ...
    async def start(self):
        ...
        # disk and used to leave the desk at warming + running=false, with
        # every seat stuck in a WAIT shell and the clock on the 1H fallback.
        ...
        if self.eth and self.eth.store is not self.btc.store:
            # Same SQLite file. A second init() can deadlock the WAL and
            # never return. ETH reads/writes through the BTC store.
            self.eth.store = self.btc.store
