...
DEFAULT_CAPS: Dict[str, float] = {
    "max_stake": 25.0,
    # NOTE: this currently caps cumulative daily *risk* (stake accepted), not
    # realized loss. DayBook.loss is read by evaluate_order but nothing writes
    # it — the Follower path has no settle hook yet. Until one exists, treat
    # this as an exposure cap and do not rely on it to stop a losing day.
...
class FollowerRuntime:
    ...
    def record_accept(self, stake: float, contracts: int) -> None:
        # Kept for compatibility with existing callers. New placement uses
        # reserve(), which checks and records under one lock.
        with self._lock:
            self.roll()
    ...
    def reserve(self, stake: float, contracts: int) -> bool:
        """Atomically reserve exposure before a live broker request."""
        with self._lock:
            self.roll()
            stake = max(0.0, float(stake))
            contracts = max(0, int(contracts))
            if stake > float(self.caps["max_stake"]):
                return False
            if self.book.loss >= float(self.caps["max_daily_loss"]):
                return False
            if self.book.risk + stake > float(self.caps["max_daily_loss"]):
                return False
            if self.book.contracts + contracts > float(self.caps["max_contracts"]):
                return False
            self.book.risk += stake
            self.book.contracts += contracts
            self.save()
            return True

    def release(self, stake: float, contracts: int) -> None:
        """Refund exposure booked by reserve() when the live route does not
        actually reach the broker. Without this, a transient broker/quote/intent
        failure would permanently consume the day's risk/contract caps and lock
        out later legitimate trades. Mirrors reserve() in reverse, clamped >= 0."""
        with self._lock:
            self.roll()
            stake = max(0.0, float(stake))
            contracts = max(0, int(contracts))
            self.book.risk = max(0.0, self.book.risk - stake)
            self.book.contracts = max(0, self.book.contracts - contracts)
            self.save()
...
class FollowerGate:
    def __init__(
        self,
        admin_password: str | Callable[[], str],
        *,
        ...
        # rotated admin secret takes effect without a restart.
        if callable(admin_password):
            self._load_p1 = admin_password
        else:
            _fixed = admin_password if isinstance(admin_password, str) else ""
            self._load_p1 = lambda: _fixed
