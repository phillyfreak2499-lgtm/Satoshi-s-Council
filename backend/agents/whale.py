...
Multi-window upgrades:
  - Tracks whale direction bias across the live window path
  - Compares current whale lean to prior-window streak / mean-reversion
  - ENTRY: clustered whale flow with enough size to bet the whole window
  - MID/FINAL: whales flipping against entry → candidate revision
...
class WhaleSpecialist(BaseSpecialist):
    ...
    def __init__(self):
        super().__init__()
        # (ts, signed_size) recent whale prints
    ...
    def _ingest_prints(self, market_data: Dict[str, Any]) -> Dict[str, float]:
        """
        Normalize various possible whale/large-trade payloads into a lean score.
        Positive = buy / YES pressure. Negative = sell / NO pressure.
        """
        now = time.time()
        signed = 0.0
        count = 0
        ...
        # Fallback proxies
        buy_v = market_data.get("buy_volume") or market_data.get("taker_buy_volume")
        sell_v = market_data.get("sell_volume") or market_data.get("taker_sell_volume")
        try:
            if buy_v is not None and sell_v is not None:
                ...
                if total > 0:
                    proxy = (bv - sv) / total
                    signed += proxy * total * 0.15  # dampened proxy contribution
        except (TypeError, ValueError):
            pass

        # Prune old prints (> 4 min)
        cutoff = now - 240
    ...
    async def get_signal(self, market_data: Dict[str, Any]) -> AgentSignal:
        ...
        if phase == "entry":
            if local_dir and (strong or clustered):
                ...
                direction, conf = local_dir, max(52, local_conf - 3)
            else:
