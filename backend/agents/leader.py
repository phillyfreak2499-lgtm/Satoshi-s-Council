        ...
        *,
        ...
        if disp != "WAIT" or book.open_legs:
            ...
            if decision.action == "DUAL":
                ...
                summary = f"{decision.action} {action} · {pos_bit} · paper · {goal_txt} · {summary}"
            elif book.open_legs:
                summary = f"Book {disp} · {action} · {pos_bit} · paper · {goal_txt} · {summary}"
            else:
                summary = f"{action} · paper · {goal_txt} · {summary}"
        else:
            firm = False
            if decision.reason in ("first_3m", "last_2_5m", "dead_book", "chalk", "size_zero"):
                summary = f"WAIT · standing down ({decision.reason}) · {goal_txt} · {summary}"
            else:
                summary = f"WAIT · no attractive book · {goal_txt} · {summary}"
        if gate_notes:
