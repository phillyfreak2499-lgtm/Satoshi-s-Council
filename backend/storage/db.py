...
class PerformanceStore:
        ...
        async with self.Session() as session:
            ...
            for row in rows:
                ...
                try:
                    ...
                    pass
                # Raw BTC spot at settlement, for the MOVE grade (independent of
                # the Kalshi strike). Additive only — never affects `correct`.
                try:
    ...
    async def add_workspace_journal_entry(self, account_id: str, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            confluence = 0
        async with self.Session() as session:
            if str(account.get("tier") or "free") == "free":
                count = await session.scalar(
                    select(func.count()).select_from(WorkspaceJournalEntry).where(
                        WorkspaceJournalEntry.account_id == account_id))
                if int(count or 0) >= int(getattr(settings, "FREE_JOURNAL_LIMIT", 10)):
                    raise ValueError("free journal limit reached; membership unlocks full history")
            now = datetime.now(timezone.utc).isoformat()
    ...
    async def workspace_snapshot(self, account_id: str) -> Dict[str, Any] | None:
        ...
        if account is None:
            return None
        cap = 100 if account.get("tier") == "member" else int(getattr(settings, "FREE_JOURNAL_LIMIT", 10))
        async with self.Session() as session:
        ...
        return {
            ...
            "membership": {
                ...
                "subscription_state": account.get("subscription_state"),
            },
        }
