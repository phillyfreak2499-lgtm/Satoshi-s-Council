...
class KalshiClient:
    def __init__(self, series_ticker: Optional[str] = None):
        ...
        self.series_ticker = series_ticker or settings.SERIES_TICKER
    ...
    async def get_open_markets(self) -> List[Dict[str, Any]]:
        ...
        params = {
            ...
            "status": "open",
            "limit": 200,  # hourly ladders can be large
        }
    ...
    def _pick_primary(self, markets: List[Dict[str, Any]], spot: Optional[float] = None) -> Optional[Dict[str, Any]]:
        """Soonest close_time; among same event, strike nearest to spot."""
        if not markets:
            return None
        markets_sorted = sorted(markets, key=lambda m: m.get("close_time") or "")
        # Group by event / close_time
        ...
        def strike_of(m: Dict[str, Any]) -> Optional[float]:
            for k in ("floor_strike", "cap_strike", "strike_price", "yes_sub_title"):
                v = m.get(k)
                if v is None:
                    continue
                try:
                    ...
                    # try parse dollars from title fragments
                    continue
            return None
    ...
    async def get_current_market_state(
        ...
        """Soonest open hourly market; prefer strike nearest to spot."""
        try:
