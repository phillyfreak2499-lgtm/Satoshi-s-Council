...
import {
  ...
  isGavelSize,
  type GavelSize,
} from "@/lib/desk/size-view";
import {
  bookState,
  bookableShadow,
  CHAIR_MIN_ASK_CENTS,
  FLOOR_SHADOW_CENTS,
} from "@/lib/desk/book-floor";
...
function PitChip({ tag }: { tag: string }) {
  const blocking =
    tag === "FEED_DOWN" || tag === "DERIVS_DOWN" || tag === "LOCKDOWN";
...
function EconomicsBox({ eco }: { eco: Economics }) {
  const cell = (label: string, value: string, tone?: string) => (
    <div key={label} className="min-w-0">
      ...
      <div className={cn("font-mono tabular text-ui", tone ?? "text-fg")}>
        {value}
      </div>
    </div>
  );
  ...
  return (
    <div className="mt-3 rounded-md border border-border bg-bg/40 px-3 py-2.5">
      ...
      <div className="mt-2 grid grid-cols-3 gap-x-4 gap-y-2.5 sm:grid-cols-4 lg:grid-cols-8">
        {cell(
          "side",
          eco.side ? (eco.side === "UP" ? "YES" : "NO") : "—",
          eco.side ? undefined : "text-wait",
        )}
        {cell("fair", c(eco.fair))}
        {cell(
          "ask",
          eco.side ? c(eco.ask) : "—",
          eco.side && !eco.bookable ? "text-wait" : undefined,
        )}
        {cell("fee", eco.side ? c(eco.fee) : "—")}
        {cell(
          "edge",
          eco.side ? signed(eco.edge) : "—",
          !eco.side ? undefined : eco.edge >= 0 ? "text-up" : "text-down",
        )}
        {cell("needs", eco.side ? `${eco.breakeven.toFixed(0)}%` : "—")}
        {cell(
          "leftover",
          signed(eco.leftover),
          eco.leftover < 0 ? "text-wait" : undefined,
        )}
        ...
        {cell(
          "touch",
          eco.side ? fmtContracts(eco.touch) : "—",
          eco.side && eco.touch <= 0 ? "text-wait" : undefined,
        )}
      </div>
...
function ChairBoard({
  ...
  const bookSide =
    book.kind === "booked" ? (book.lean === "UP" ? "YES" : "NO") : side;
  const tone =
    lean === "UP" ? "text-up" : lean === "DOWN" ? "text-down" : "text-wait";
  const barTone =
    lean === "UP" ? "bg-up" : lean === "DOWN" ? "bg-down" : "bg-wait";
  const edge =
    lean === "UP" ? snap.edge_up : lean === "DOWN" ? snap.edge_down : 0;
  ...
  return (
    <section
      ...
      className={cn(
        ...
        lean === "UP"
          ? "border-up/40 shadow-[0_0_0_1px_rgba(61,207,138,0.12)]"
          : lean === "DOWN"
            ? "border-down/40 shadow-[0_0_0_1px_rgba(239,107,115,0.12)]"
            : "border-border",
      )}
    >
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="min-w-0">
          ...
          <div
            className={cn(
              "font-sans text-hero font-medium leading-none tracking-tight",
              tone,
            )}
            aria-live="polite"
            aria-atomic="true"
          ...
          <p
            className="mt-1.5 max-w-[52ch] font-sans text-ui leading-snug text-fg"
            data-plain-line
          >
          ...
          <p className="mt-1 max-w-[52ch] font-sans text-ui leading-snug text-muted">
            {book.kind === "booked"
              ? `Paper only: booked ${book.lean} at ${book.cents.toFixed(0)}¢ on the ${bookSide} ask, held to settlement and graded on Kalshi's official value.${
                  lean !== book.lean
                    ? " The read has moved since; one position per window means the book does not sell low to buy high."
                    : ""
                }`
              : lean === "WAIT"
          ...
          <div className="mt-2 font-mono text-ui text-muted">
            {book.kind === "booked" ? (
              <>
                {book.lean} booked {book.cents.toFixed(1)}¢ · {bookSide} ask now{" "}
                {book.ask.toFixed(1)}¢
              </>
            ) : lean === "WAIT" ? (
              "no paper fill"
                  ...
                  <span className="text-wait">
                    ...
                    {bookableShadow(book.ask) ? (
                      <span className="text-subtle">
                        {" "}
                        · {FLOOR_SHADOW_CENTS}¢ shadow books it
                      </span>
                    ) : null}
                  </span>
                ) : null}
        ...
        <div className="flex flex-wrap items-end gap-6">
          ...
          <div>
            ...
            <div
              className={cn(
                "font-mono text-call leading-none",
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        ...
        {failedGates.map((g) => (
          <span
            key={g.id}
            className="rounded-sm border border-down/40 bg-down/10 px-1.5 py-px font-mono text-micro uppercase tracking-wide text-down"
      ...
      {mathOpen ? (
        <div className="mt-1.5 space-y-0.5 border-t border-border pt-1.5 font-mono text-micro text-subtle">
          <div>
            sit-mass {chair.sit_mass.toFixed(2)} · agg ×
            {chair.aggressiveness.toFixed(2)} · diversity ×
            {chair.diversity.toFixed(2)} · split{" "}
            {chair.conflict_frac.toFixed(2)}
          </div>
    ...
    <section className="rounded-md border border-dashed border-border bg-surface p-3">
      ...
      <div className="mt-2 flex flex-wrap items-end gap-x-6 gap-y-2">
        <div>
          ...
          <div
            className={cn("font-mono text-title tabular leading-none", tone)}
          >
        ...
        <div>
          ...
          {live ? (
            <div className="mt-1 font-mono text-micro text-muted">
              edge{" "}
              {live.edge_cents == null
                ? "— (no book)"
                : `${fmtCents(live.edge_cents)} after fee`}
            </div>
          ) : null}
        </div>
        {st ? (
          <div className="font-mono text-micro text-muted">
            <div>
              v2 {st.calls_v2} calls · net {fmtCents(st.ev_v2)} &nbsp;|&nbsp;
              chair {st.calls_v1} calls · net {fmtCents(st.ev_v1)}
            </div>
            <div>
              Brier v2 {st.brier_v2 == null ? "—" : st.brier_v2.toFixed(3)} ·
              market{" "}
              {st.brier_market == null ? "—" : st.brier_market.toFixed(3)} ·{" "}
              {st.n_graded} graded windows
            </div>
          </div>
        ) : null}
      </div>
      {st ? (
        <div className="mt-2 font-mono text-micro text-muted">
          promotion gate {v2Gates(st).met}/3 · samples {st.n_graded}/
          {V2_GATE_SAMPLES} · calls {st.calls_v2}/{V2_GATE_CALLS} with net &gt;
          0 · Brier {v2Gates(st).brierOk ? "beats" : "trails"} market
        </div>
      ) : null}
      {v2.top.length ? (
        <div className="mt-2 truncate font-mono text-micro text-subtle">
          weights ·{" "}
          {v2.top
            .map(([k, w]) => `${k} ${w >= 0 ? "+" : ""}${w.toFixed(2)}`)
            .join(" · ")}
        </div>
      ) : null}
      <p className="mt-2 font-mono text-micro text-subtle">
        Paper only. One probability learned from the ledger — every seat&apos;s
        honest read plus the market — trading only where it beats the ask by
        more than the fee, only as far from the market as its calibration record
        has earned, and never under 35¢. It competes with the chair on identical
        windows and is promoted only if it wins.
      </p>
    </section>
  );
}
...
function OvernightRibbon({ brief, tz }: { brief: Brief | null; tz: string }) {
  ...
  const copyLine = (() => {
    if (!o) return "";
    const btc =
      o.btc_open != null && o.btc_now != null ? `BTC ${open}→${now}` : "BTC —";
    const line =
      o.up + o.down === 0
        ? `Overnight: Chair WAIT x${o.wait}. ${btc}. No chase.`
        : `Overnight: Chair ${o.up}U/${o.down}D/${o.wait}W. ${btc}.`;
          ...
          <span className="flex flex-wrap items-center gap-x-2 gap-y-1 font-mono text-data tabular">
            ...
            {o.last ? (
              <>
                <span className="text-subtle">·</span>
                <span className="text-muted">
                  last {hhmm(o.last.t, tz)}{" "}
                  {o.last.settle == null ? "" : `${o.last.settle.toFixed(0)}¢ `}
                  Chair {o.last.lean}
                </span>
              </>
            ) : null}
...
function ChairScoreboard({ v2 }: { v2?: V2Frame | null }) {
  ...
  const cents = (n: number | null | undefined) =>
    n == null || !Number.isFinite(n)
      ? "—"
      : `${n >= 0 ? "+" : ""}${n.toFixed(0)}¢`;
  const brier = (n: number | null | undefined) =>
    n == null || !Number.isFinite(n) ? "—" : n.toFixed(3);
  const bothBrier = st && st.brier_v2 != null && st.brier_market != null;
  return (
    <section
      aria-label="Chair record"
      className="rounded-md border border-border bg-surface px-3 py-2 font-mono text-micro"
    >
      <span className="uppercase tracking-widest text-subtle">
        Chair record
      </span>{" "}
      <span className="text-muted">
        <span className="text-fg tabular">{st ? st.calls_v1 : "—"}</span> calls
        · net{" "}
        <span
          className={cn(
            "tabular",
            st && st.ev_v1 >= 0 ? "text-up" : st ? "text-down" : "text-subtle",
          )}
        >
          {cents(st?.ev_v1)}
        </span>{" "}
        ...
        <span className="tabular text-fg">
          {bothBrier ? brier(st!.brier_v2) : "—"}
        </span>{" "}
        vs market{" "}
        <span className="tabular text-fg">
          {bothBrier ? brier(st!.brier_market) : "—"}
        </span>{" "}
        · gate{" "}
        <span className="tabular text-fg">
          {st ? `${st.calls_v1}/${st.n_graded}` : "—/—"}
        </span>
      </span>
      <span className="ml-2 text-subtle">
        · UP/DOWN calls only; WAIT is a decision, not a fill.
      </span>
    </section>
  );
}
    ...
    <section className="rounded-md border border-border bg-surface">
            ...
            >
              {GAVEL_SIZES.map((n) => (
                <option key={n} value={n}>
                  {n === 1
                    ? "1 contract"
                    : `${n.toLocaleString("en-US")} contracts`}
                </option>
              ))}
            </select>
          </label>
          ...
          <table className="table-research px-3">
            <thead>
              <tr>
                ...
                <th>call</th>
                <th className="num">
                  <Tip k="strip.conf">decision conf</Tip>
                </th>
            ...
            <tbody>
              {gavel.map((g, i) => {
                ...
                return (
                  <tr key={`${g.t}-${i}`}>
                    <td className="whitespace-nowrap pl-3 text-muted">
                      {hhmm(g.t, tz)}
                    </td>
                    <td>
                      <span
      ...
      {size > 1 ? (
        <p className="border-t border-border px-3 py-1.5 font-mono text-micro text-subtle">
          The same fills at {size.toLocaleString("en-US")} contracts: fee at
          size, rounded once per order, and the ask assumed to hold — real size
...
function SeatsList({
  ...
  rows: SeatRow[];
  learner: import("@/lib/desk/types").Learner;
}) {
  ...
  return (
    <details className="group rounded-md border border-border bg-surface">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-3 py-2.5 font-mono text-micro uppercase tracking-widest text-subtle marker:content-none hover:text-fg">
        <span>
          <Tip k="seats.list">SEATS — specialist paper fills</Tip> ·{" "}
          {withFills.length} with fills
        </span>
        <span
          aria-hidden="true"
      ...
      <div className="border-t border-border px-3 py-2">
        <p className="mb-2 font-mono text-micro text-subtle">
          Each seat&apos;s own paper scalps in cents — specialist practice, not
          the Chair&apos;s book. These do not settle windows and are never Chair
          calls.
        </p>
        {!withFills.length ? (
          <div className="font-mono text-ui text-muted">
            No specialist fills yet.
          </div>
        ) : (
                  ...
                  <tr key={r.seat}>
                    ...
                    <td className="font-mono text-micro tabular text-subtle">
                      {st.legs.slice(-8).map((c, i) => (
                        <span
                          key={i}
                          className={c >= 0 ? "text-up" : "text-down"}
                        >
                          {c >= 0 ? "+" : ""}
                          {c.toFixed(0)}
...
export function SatoshiTab({
  ...
  const speaking = chair.rows.filter(
    (r) => r.lean === "UP" || r.lean === "DOWN",
  ).length;
  ...
  const lastSettled =
    [...callLog]
      .filter((r) => r.settle != null)
      .sort((a, b) => b.close_time - a.close_time)[0] ?? null;
  const rows = chair.rows.filter((r) =>
    view === "all"
      ? true
      : view === "speaking"
        ? r.lean === "UP" || r.lean === "DOWN"
        : r.status === "LIVE",
  );
  return (
    <div className="gutter mx-auto flex w-full max-w-[var(--max)] flex-col gap-4 py-4">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <ArenaPanel
          snap={snap}
          live={settings.source === "live"}
          onOpenArena={onOpenArena ?? (() => {})}
        />
        <LastReplayCard
          ticker={lastSettled?.ticker ?? null}
          at={
            lastSettled ? new Date(lastSettled.close_time).toISOString() : null
          }
      ...
      <details className="group rounded-md border border-border bg-surface">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-3 py-2.5 font-mono text-micro uppercase tracking-widest text-subtle marker:content-none hover:text-fg">
          <span>
            Diagnostics · chair v2, the score math, and the full seat table
          </span>
          <span
            aria-hidden="true"
            className="transition-transform duration-200 ease-out group-open:rotate-90"
        ...
        <div className="flex flex-col gap-3 border-t border-border p-3">
          ...
          <div className="flex flex-wrap items-center gap-2 font-mono text-micro">
            <span className="text-subtle">
              <Tip k="pane.seats">the voting seats</Tip> · {speaking} speaking ·{" "}
              {chair.rows.length - speaking} sitting
            </span>
            <div
              role="group"
              aria-label="Which seats to show"
          ...
          <div className="overflow-x-auto rounded-md border border-border">
            <table className="w-full min-w-[72rem] text-left">
              ...
              <tbody>
                {rows.map((r) => {
                  const maxC = Math.max(
                    ...chair.rows.map((x) => Math.abs(x.contribution)),
                    0.001,
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {r.scalp_avg == null
                          ? "—"
                          : `${r.scalp_avg >= 0 ? "+" : ""}${r.scalp_avg.toFixed(1)}`}
                        {r.scalp_n ? (
                          <span className="text-subtle"> · {r.scalp_n}</span>
                        ) : null}
                      </td>
                      <td className="px-2 py-1 font-mono text-data tabular text-muted">
                      ...
                      <td className="px-2 py-1 font-mono text-data tabular text-fg">
                        {r.calls}
                      </td>
                      ...
                      <td className="px-2 py-1 font-mono text-data text-muted">
                        {r.callsign}
                      </td>
                      <td className="px-2 py-1">
                        <LeanChip lean={r.lean} cents={sideAsk(snap, r.lean)} />
                      </td>
                      <td className="px-2 py-1 font-mono text-data tabular">
                        {r.conf}
                      </td>
                      ...
                      {settings.show_shadow && (
                        <td className="px-2 py-1">
                          {r.shadow_lean ? (
                            <LeanChip
                              lean={r.shadow_lean}
                              cents={sideAsk(snap, r.shadow_lean)}
                            />
                          ) : (
                            <span className="text-subtle">—</span>
                          )}
                      ...
                      <td
                        className="max-w-xs truncate px-2 py-1 font-sans text-ui text-muted"
                        title={r.why}
                      >
                        {r.why}
                      </td>
          ...
          <div className="grid gap-3 lg:grid-cols-3">
            <Pane title={<Tip k="pane.score">Score math</Tip>}>
              <div className="space-y-1 font-mono text-data">
                ...
                <div>
                  sit-mass {chair.sit_mass.toFixed(2)} → bar +
                  {(0.2 * chair.sit_mass).toFixed(3)}
                </div>
                <div>
                ...
                <div>
                  confluence bar {chair.bar.toFixed(2)} (floor 0.24 / ceil 0.72)
                </div>
                <div>aggressiveness ×{chair.aggressiveness.toFixed(2)}</div>
                <div>
                  diversity ×{chair.diversity.toFixed(2)} (
                  {chair.categories_agree} cats)
                </div>
                ...
                {chair.walk ? (
                  <div>
                    walk-forward n={chair.walk.n} · train{" "}
                    {Math.round(chair.walk.train_hit * 100)}%{" "}
                    {chair.walk.train_ev >= 0 ? "+" : ""}
                    {chair.walk.train_ev.toFixed(1)}¢ · later{" "}
                    {Math.round(chair.walk.test_hit * 100)}%{" "}
                ...
                <div>
                  full-call conf min(92, round(50+|score|×55)) ={" "}
                  {chair.full_conf_raw}
                  {chair.lean === "WAIT" ? " · WAIT uses gate conf" : ""} →{" "}
                  {chair.confidence} conf
                  {chair.lean !== "WAIT"
                    ? ` · call ${sideAsk(snap, chair.lean).toFixed(0)}¢`
                    : ""}
                </div>
              </div>
            </Pane>
            <Pane title={<Tip k="pane.gates">Gate checklist</Tip>}>
              <ul className="space-y-1">
                {chair.gates.map((g) => (
                  <li
                    key={g.id}
                    className="flex items-start justify-between gap-2 font-mono text-data"
                  >
                    <span
                      className={
                        g.pass ? "text-up" : g.hard ? "text-down" : "text-wait"
                      }
                    >
                      {g.pass ? "PASS" : "FAIL"}
                    </span>
            ...
            <Pane title={<Tip k="pane.thinking">Chair thinking</Tip>}>
              <div className="space-y-1.5">
                ...
                <Field
                  ...
                  v={
                    <ul className="space-y-0.5">
                      {chair.evidence.length
                        ? chair.evidence.map((e) => <li key={e}>{e}</li>)
                        : "—"}
                    </ul>
                  }
                />
                ...
                <Field
                  k="invalidate if"
                  v={invalidateCondition(chair.invalidate_if)}
                />
                <Field
                  k="calc"
                  v={<span className="font-mono text-data">{chair.calc}</span>}
                />
                <Field
                  k="skill / huddle"
                  v={`${chair.last_settle} / ${chair.huddle_line}`}
                />
              </div>
            </Pane>
          </div>
    ...
    <div className="border-t border-border bg-surface">
      ...
      {tape[0] && (
        <div className="border-t border-border px-3 py-1 font-mono text-micro text-muted">
          ...
          <span
            className={
              settling || tape[0].startsWith("PENDING")
                ? "text-wait"
                : undefined
            }
          >
