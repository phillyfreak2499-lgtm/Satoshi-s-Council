...
import { publicLastWindow, publicWeekBooks } from "@/lib/desk/record-public";
...
export const Route = createFileRoute("/")({
  ...
  loader: async () => {
    const [frame, last, week] = await Promise.all([publicHomeSnapshot(), publicLastWindow().catch(() => null), publicWeekBooks().catch(() => null)]);
    return { frame, last, week };
  },
  component: Home,
});
...
function Home() {
  ...
  const { frame, last, week } = Route.useLoaderData();
  return <InitialDeskFrame.Provider value={frame}>{legacyRoom ? <DeskApp /> : <CouncilHome last={last} week={week} />}</InitialDeskFrame.Provider>;
}
