...
test("chair fold contract keeps one authorized LIVE representative", () => {
  type Acc = { seat: SeatId; status: SeatRow["status"]; health: SeatRow["health"]; w: number };
  const fam: Acc[] = [
    { seat: "CHAIN", status: "LIVE", health: "LIVE", w: 0.30 },
    { seat: "CARRY", status: "LIVE", health: "LIVE", w: 0.40 },
    { seat: "CASCADE", status: "UNCALIBRATED", health: "LIVE", w: 0.20 },
  ];
  const representative = pickAuthorizedRepresentative(fam, {
    authorized: (a) => (a.status === "LIVE" || a.status === "FADED") && a.health === "LIVE",
    weight: (a) => a.w,
    seat: (a) => a.seat,
  });
  assert.equal(representative?.seat, "CARRY");
  for (const a of fam) {
    if (a === representative) continue;
    if (a.status === "LIVE" || a.status === "UNCALIBRATED") a.status = "FOLDED";
  }
  const rows = fam.map((a) => row(a.seat, { status: a.status, folded: true, health: a.health }));
  const support = familySupport([...rows, row("STREAK")], "UP");
  assert.deepEqual(support.seats, ["CARRY", "STREAK"]);
  assert.equal(support.familyCount, 2);
  assert.equal(selectiveBlock(snap(), chairOf([...rows, row("STREAK")]), ctx()), null);
});
