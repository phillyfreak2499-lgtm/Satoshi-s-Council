...
test("the deployed ledger insert preserves old rows and stores new receipts atomically", async () => {
  ...
  try {
    ...
    await db.query(sql, [...values, JSON.stringify(audit)]);
    await db.query(sql, [...values, null]); // retry never overwrites the first receipt
    const saved = await db.query<{ skill_score_audit: unknown }>("select skill_score_audit from desk_ledger where ticker=$1", [snap().ticker]);
    assert.deepEqual(saved.rows[0]!.skill_score_audit, audit);
    values[0] = "AUDIT-RESTORED";
    await db.query(sql, withSkillAuditColumn(values));
