...
3) Where the floor belongs. The live floor is FLOOR_LIVE_CENTS in book-floor.ts, currently 80¢, raised from 70¢ as a time-boxed trial at FLOOR_LIVE_SINCE; FLOOR_SHADOW_CENTS (70¢) keeps running beside it as a research book on the same windows, in desk_ledger.shadow_entry_cents / shadow_ev_cents. On out-of-sample windows, compare the fills by price shelf — 70–79¢ against 80¢ and up — on win rate against the breakeven each shelf needed (entry plus fee), net cents, and drawdown, broken down by regime, and compare the live book against the shadow book on the trial windows only. Is 80¢ the right line, does the edge begin higher still, or did 70¢ deserve its place? Report the shelves and both books with their counts, and say plainly when the sample is too thin to separate them; whether the floor moves again is a separate sign-off.
