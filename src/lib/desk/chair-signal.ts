...
export function signalDescription(signal: ChairSignal | null): string {
  ...
  return `Current Chair frame: raw lean ${signal.score >= 0 ? "+" : ""}${signal.score.toFixed(3)}, ${pct.toFixed(0)}% of the fixed directional-strength scale toward ${signal.direction}. Current call line ${signal.rawThreshold == null ? "unreachable" : `±${signal.rawThreshold.toFixed(3)} raw score`}; effective signal ${signal.effective.toFixed(3)} vs bar ${signal.threshold.toFixed(3)}. ${relation} Standing Chair read: ${signal.decision}. A standing UP/DOWN read can come from an earlier qualifying frame while this live marker and the current call lines keep moving. If a decision snapshot is recorded above, that is the call-time frame. Paper entry remains separate.${Math.abs(signal.directionalFraction) > 1 ? " Marker is at the end of the scale; the numeric reading is not clipped." : ""}`;
}
