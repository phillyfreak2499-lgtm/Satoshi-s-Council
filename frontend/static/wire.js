...
window.COUNCIL_WIRE = [
  {
    ...
    "why": "Zach Voice: HOLD the old win rule. BTC 15m is path P&L, not one irreversible directional lock. A dual-sided scalp: hold both Up and Down when combined cost is attractive. Both legs only when UP ask + DOWN ask leaves room after vig. Paper fill at the real ask, not mid. Dead 99¢ book = sit. You cannot scale out of chalk. Scale / cut / flip inside the 15m window — BTC 15m only. Score realized paper P&L, not close-direction hits. Do not treat a single UP/DOWN lock that matches the official settle as the training win. A new brain — not a clock change. Satoshi + Bitcoin Pattern Specialist retrained on 15m path behavior. Retrain on old 15-minute books only. Do not port 1H weights. CoinGlass 1h stays off this book. ETH stays 1H. ETH 1H stays one-lock — not dual-sided. Playable band stays 20–80 after vig. WAIT is a skip, not a miss. 99¢ chalk is not a training win. Displayed BTC hits start clean — 15m scorecard is path P&L, not a directional hit. ETH displayed hits and ETH 1H bot memory stay. Vitalik / candle_eth stay hourly. Dedicated specialists stay asset-pure. Shared bots never final-lock. Hunter still feeds 1–3; Ares/Oracle paper-lock the best of three. Oracle laurel still. Cowboy Raijin. Signed WAIT stills + cache-bust. Gate CSS #41. Logo /council-mark.png. Header STILL. No new Floor chairs. Paper. Follower OFF. Live OFF."
  },
  {
