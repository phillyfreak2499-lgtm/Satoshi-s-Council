...
class WindowCall(Base):
    ...
    # Raw BTC spot when the call was made and when its window settled. Used to
    # grade the actual move ("buy → did price rise") rather than finish-vs-strike.
    spot_at_call: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    spot_at_close: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
...
def _workspace_text(value: Any, limit: int, field: str, *, required: bool = False) -> str:
    text = str(value or "").strip()
    if required and len(text) < 8:
        raise ValueError(f"{field} needs at least 8 characters")
    if len(text) > limit:
...
# Bitcoin-only journal vocabulary (personal research records).
WORKSPACE_ASSETS = {"btc", "eth"}          # ETH kept only as a signal note
WORKSPACE_STANCES = {"ACCUMULATE", "REDUCE", "MAINTAIN", "WAIT"}
WORKSPACE_HORIZONS = {"15m", "1h", "4h", "1d"}


class WorkspaceAccount(Base):
    """Anonymous-by-default account, ready for authenticated membership."""
    ...
    subscription_state: Mapped[str] = mapped_column(String(24), default="inactive")
    stripe_customer_id: Mapped[Optional[str]] = mapped_column(String(128), nullable=True, unique=True)
    stripe_subscription_id: Mapped[Optional[str]] = mapped_column(String(128), nullable=True, unique=True)
    created_at: Mapped[str] = mapped_column(String(40))
    updated_at: Mapped[str] = mapped_column(String(40))


class WorkspaceJournalEntry(Base):
    """User-owned research process record; it never routes or sizes an order."""
    __tablename__ = "workspace_journal_entries"
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    account_id: Mapped[str] = mapped_column(String(48), index=True)
    ...
    next_review_at: Mapped[Optional[str]] = mapped_column(String(40), nullable=True)
...
class WeeklyProcessReview(Base):
    """One process score per account/week, independent of market P&L."""
    __tablename__ = "weekly_process_reviews"
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    account_id: Mapped[str] = mapped_column(String(48), index=True)
    week_key: Mapped[str] = mapped_column(String(16), index=True)
    horizon_named: Mapped[int] = mapped_column(Integer, default=0)
    invalidation_named: Mapped[int] = mapped_column(Integer, default=0)
    review_honored: Mapped[int] = mapped_column(Integer, default=0)
    wait_respected: Mapped[int] = mapped_column(Integer, default=0)
...
class PerformanceStore:
    ...
    async def init(self):
        ...
        async with self.engine.begin() as conn:
            ...
            for stmt in (
                ...
                # Raw BTC spot at call time + at settlement, for MOVE accuracy
        ...
        async with self.Session() as session:
            for fill in fills:
                ...
                session.add(WindowCall(
                    ...
                    sizing=sizing_txt,
                    # Raw BTC spot for the chalk-immune MOVE grade. `spot` is the
                    # threaded live price; entry_price is the same BTC spot in this
                    # pipeline, so fall back to it (guarded >1000 so a stray Kalshi
                    # cents value can never masquerade as spot).
                    spot_at_call=(
                        float(spot) if isinstance(spot, (int, float)) and spot
                        else (float(entry_price)
                              if isinstance(entry_price, (int, float)) and entry_price and entry_price > 1000
                              else None)
                    ),
                ))
            await session.commit()
        ...
        async with self.Session() as session:
            ...
            for row in rows:
                ...
                try:
                    ...
                    pass
                # Raw BTC spot near the window close. Do not stamp a late
                # processing-time print as if it were the close.
                try:
                    if isinstance(current_price, (int, float)) and current_price:
                        close_ok = True
                        if row.close_time:
                            ct = datetime.fromisoformat(str(row.close_time).replace("Z", "+00:00"))
                            if ct.tzinfo is None:
                                ct = ct.replace(tzinfo=timezone.utc)
                            close_ok = abs((now - ct).total_seconds()) <= 180
                        if close_ok:
                            row.spot_at_close = float(current_price)
                except Exception:
                    pass
    ...
    async def recent_settled_calls(self, limit: int = 20, asset: str | None = None) -> List[Dict[str, Any]]:
        ...
        async with self.Session() as session:
            ...
            for r in rows:
                votes = None
                called = r.called_at or ""
                q = select(SignalRecord).where(SignalRecord.market_ticker == r.ticker)
                if called:
                    q = q.where(SignalRecord.timestamp <= called)
                sig = (
                    await session.execute(
                        q.order_by(SignalRecord.timestamp.desc(), SignalRecord.id.desc()).limit(1)
                    )
                ).scalar_one_or_none()
    ...
    async def get_manual_journal(self, owner: str | None = None) -> Dict[str, Any]:
        ...
        async with self.Session() as session:
            # Only this visitor's rows. Legacy owner-NULL rows stay unscoped
            # so they are not disclosed to every later workspace cookie.
            scope = (
                ManualTrade.owner == str(owner)
    ...
    async def move_accuracy(self, asset: str = "btc") -> Dict[str, Any]:
        """
        MOVE accuracy: did BTC's raw spot move the way the call said, from the
        moment of the call to settlement — independent of the Kalshi strike.
        UP is correct if spot rose; DOWN is correct if spot fell. Only calls that
        recorded BOTH spots count, and a tiny flat band (<0.02%) is ignored.

        This is the "buy → did price go up" grade; it excludes chalk (a call that
        finished the right side of the strike while price actually moved against
        it). Additive and read-only. Paper research only.
        """
        want = (asset or "btc").strip().lower()
        async with self.Session() as session:
            rows = (await session.execute(
                select(WindowCall)
                .where(WindowCall.spot_at_call.isnot(None),
                       WindowCall.spot_at_close.isnot(None))
                .order_by(WindowCall.id.desc())
                .limit(20000)
            )).scalars().all()
        up_n = up_hit = dn_n = dn_hit = 0
        for r in rows:
            ra = (r.asset or ticker_asset(r.ticker) or "").lower()
            if want and ra and ra != want:
                continue
            ...
            if d in ("UP", "UP_HOLD", "LONG_UP"):
                ...
                continue
            try:
                p0 = float(r.spot_at_call); p1 = float(r.spot_at_close)
            except (TypeError, ValueError):
                continue
            if p0 <= 0:
                continue
            move = p1 - p0
            if abs(move) < p0 * 0.0002:   # ignore <0.02% flat
                continue
            if is_up:
                up_n += 1; up_hit += 1 if move > 0 else 0
            else:
                dn_n += 1; dn_hit += 1 if move < 0 else 0
        total = up_n + dn_n
        hits = up_hit + dn_hit

        def pct(h, n):
            return round(100.0 * h / n, 1) if n else None
        return {
            "n": total,
            "correct": hits,
            "pct": pct(hits, total),
            "buy": {"n": up_n, "correct": up_hit, "pct": pct(up_hit, up_n)},
            "sell": {"n": dn_n, "correct": dn_hit, "pct": pct(dn_hit, dn_n)},
            "note": "Graded on the raw BTC move from call to settlement "
                    "(buy → price up, sell → price down), independent of the "
                    "Kalshi strike. Builds as new calls settle.",
        }

    # ── Workspace journal + membership entitlements ──────────────────────
    @staticmethod
    def _account_payload(row: "WorkspaceAccount") -> Dict[str, Any]:
        return {
            "id": row.id,
    ...
    @staticmethod
    def _workspace_entry_payload(row: "WorkspaceJournalEntry") -> Dict[str, Any]:
        return {
            "id": row.id, "asset": row.asset, "stance": row.stance, "horizon": row.horizon,
            "thesis": row.thesis, "evidence": row.evidence, "invalidation": row.invalidation,
            "next_review_at": row.next_review_at, "confluence_score": row.confluence_score,
            "status": row.status, "reflection": row.reflection,
            "created_at": row.created_at, "updated_at": row.updated_at,
        }

    @staticmethod
    def _review_payload(row: "WeeklyProcessReview") -> Dict[str, Any]:
        return {
            "id": row.id, "week_key": row.week_key,
            "horizon_named": bool(row.horizon_named), "invalidation_named": bool(row.invalidation_named),
            "review_honored": bool(row.review_honored), "wait_respected": bool(row.wait_respected),
            "score": row.score, "note": row.note,
            "created_at": row.created_at, "updated_at": row.updated_at,
        }

    async def get_or_create_workspace_account(self, account_id: str, display_name: Any = None) -> Dict[str, Any]:
        account_id = _workspace_text(account_id, 48, "account id", required=True)
        name = _workspace_text(display_name, 80, "display name") or None
        now = datetime.now(timezone.utc).isoformat()
        async with self.Session() as session:
            row = await session.get(WorkspaceAccount, account_id)
            if row is None:
                row = WorkspaceAccount(id=account_id, display_name=name, tier="free",
                                       subscription_state="inactive", created_at=now, updated_at=now)
                session.add(row)
            elif name:
    ...
    async def workspace_account(self, account_id: str) -> Dict[str, Any] | None:
        async with self.Session() as session:
            row = await session.get(WorkspaceAccount, account_id)
            return self._account_payload(row) if row else None

    async def add_workspace_journal_entry(self, account_id: str, body: Dict[str, Any]) -> Dict[str, Any]:
        account = await self.workspace_account(account_id)
        if account is None:
            raise ValueError("workspace account not found")
        asset = str(body.get("asset") or "btc").lower().strip()
        if asset not in WORKSPACE_ASSETS:
            raise ValueError("asset must be BTC (ETH allowed as a signal note)")
        stance = str(body.get("stance") or "WAIT").upper().strip().replace(" ", "_")
        # Accept the canonical set; map a couple of legacy labels for safety.
        stance = {"BUY_ZONE": "ACCUMULATE", "HOLD": "MAINTAIN", "SELL": "REDUCE",
                  "STAND_DOWN": "WAIT"}.get(stance, stance)
        if stance not in WORKSPACE_STANCES:
            raise ValueError("choose a valid stance: Accumulate / Reduce / Maintain / Stand down")
        horizon = str(body.get("horizon") or "1h").lower().strip()
        if horizon not in WORKSPACE_HORIZONS:
            raise ValueError("horizon must be 15m, 1h, 4h, or 1d")
        thesis = _workspace_text(body.get("thesis"), 1200, "thesis", required=True)
        evidence = _workspace_text(body.get("evidence"), 1200, "evidence") or None
        invalidation = _workspace_text(body.get("invalidation"), 800, "invalidation", required=True)
        review_at = _workspace_text(body.get("next_review_at"), 40, "next review") or None
        if review_at:
            try:
                datetime.fromisoformat(review_at.replace("Z", "+00:00"))
            except ValueError as exc:
                raise ValueError("next review must be an ISO date/time") from exc
        try:
            confluence = max(0, min(4, int(body.get("confluence_score") or 0)))
        except (TypeError, ValueError):
            confluence = 0
        async with self.Session() as session:
            count = await session.scalar(
                select(func.count()).select_from(WorkspaceJournalEntry).where(
                    WorkspaceJournalEntry.account_id == account_id))
            cap = int(getattr(settings, "FREE_JOURNAL_LIMIT", 2000) or 2000)
            if int(count or 0) >= cap:
                raise ValueError("journal is full")
            now = datetime.now(timezone.utc).isoformat()
            row = WorkspaceJournalEntry(
                account_id=account_id, asset=asset, stance=stance, horizon=horizon,
                thesis=thesis, evidence=evidence, invalidation=invalidation,
                next_review_at=review_at, confluence_score=confluence,
                status="open", created_at=now, updated_at=now)
            session.add(row)
            await session.commit()
            await session.refresh(row)
            return self._workspace_entry_payload(row)

    async def complete_workspace_journal_entry(self, account_id: str, entry_id: int, reflection: Any) -> Dict[str, Any]:
        note = _workspace_text(reflection, 1200, "reflection", required=True)
        async with self.Session() as session:
            result = await session.execute(
                select(WorkspaceJournalEntry).where(
                    WorkspaceJournalEntry.id == int(entry_id),
                    WorkspaceJournalEntry.account_id == account_id))
            row = result.scalar_one_or_none()
            if row is None:
    ...
    async def save_weekly_process_review(self, account_id: str, body: Dict[str, Any]) -> Dict[str, Any]:
        account = await self.workspace_account(account_id)
        if account is None:
            raise ValueError("workspace account not found")
        now = datetime.now(timezone.utc)
        ...
        checks = {
            "horizon_named": _truthy(body.get("horizon_named")),
            "invalidation_named": _truthy(body.get("invalidation_named")),
            "review_honored": _truthy(body.get("review_honored")),
            "wait_respected": _truthy(body.get("wait_respected")),
        }
        score = int(round(sum(checks.values()) / len(checks) * 100))
        note = _workspace_text(body.get("note"), 1200, "weekly note") or None
        stamp = now.isoformat()
        async with self.Session() as session:
            result = await session.execute(
                select(WeeklyProcessReview).where(
                    WeeklyProcessReview.account_id == account_id,
                    WeeklyProcessReview.week_key == week))
            row = result.scalar_one_or_none()
            if row is None:
                row = WeeklyProcessReview(account_id=account_id, week_key=week, created_at=stamp, updated_at=stamp)
                session.add(row)
    ...
    async def workspace_snapshot(self, account_id: str) -> Dict[str, Any] | None:
        account = await self.workspace_account(account_id)
        if account is None:
            return None
        cap = int(getattr(settings, "FREE_JOURNAL_LIMIT", 2000) or 2000)
        async with self.Session() as session:
            entries = list((await session.execute(
                select(WorkspaceJournalEntry)
                .where(WorkspaceJournalEntry.account_id == account_id)
                .order_by(WorkspaceJournalEntry.id.desc()).limit(max(1, cap)))).scalars().all())
            reviews = list((await session.execute(
                select(WeeklyProcessReview)
                .where(WeeklyProcessReview.account_id == account_id)
                .order_by(WeeklyProcessReview.id.desc()).limit(12))).scalars().all())
        open_entries = sum(1 for row in entries if row.status == "open")
        reviewed = sum(1 for row in entries if row.status == "reviewed")
        return {
            "account": account,
            "journal_limit": cap,
            "entries": [self._workspace_entry_payload(row) for row in entries],
            "reviews": [self._review_payload(row) for row in reviews],
            "process": {
                "entries": len(entries),
                "open_entries": open_entries,
                "reviewed_entries": reviewed,
                "review_completion_pct": round(100 * reviewed / len(entries)) if entries else None,
                "latest_weekly_score": reviews[0].score if reviews else None,
            },
            "membership": {
                "tier": "free",
    ...
    # Stripe <-> WorkspaceAccount entitlement glue (server-side; no HMAC cookie).
    async def link_stripe_customer(self, account_id: str, customer_id: str,
        ...
        async with self.Session() as session:
            ...
            if row is None:
                return
            row.stripe_customer_id = str(customer_id) if customer_id else row.stripe_customer_id
            if subscription_id:
    ...
    async def account_id_for_customer(self, customer_id: str) -> str | None:
        async with self.Session() as session:
            row = (await session.execute(
                select(WorkspaceAccount).where(WorkspaceAccount.stripe_customer_id == str(customer_id))
            )).scalar_one_or_none()
            return row.id if row else None

    async def set_membership(self, account_id: str, tier: str, subscription_state: str) -> None:
        now = datetime.now(timezone.utc).isoformat()
        async with self.Session() as session:
            row = await session.get(WorkspaceAccount, account_id)
            if row is None:
                return
            row.tier = str(tier or "free")
            row.subscription_state = str(subscription_state or "inactive")
            row.updated_at = now
            await session.commit()
    ...
    async def export_brain_rows(self, limit: int = 5000) -> list:
        ...
        async with self.Session() as session:
            result = await session.execute(
                select(WindowCall).order_by(WindowCall.id.desc()).limit(limit)
            )
            rows = result.scalars().all()
            out = []
            for r in rows:
    ...
    async def import_brain_rows(self, rows: list, mode: str = "merge") -> dict:
        ...
        for row in rows:
            ...
            try:
                ...
                return {"imported": 0, "skipped": 0, "error": "invalid confidence", "mode": mode}
            parsed.append((row, conf))
        imported = 0
        skipped = 0
        async with self.Session() as session:
            if mode == "replace":
