...
| Piece | Role |
| --- | --- |
| FastAPI backend | Continuous analysis loop, agents, Leader confluence, SQLite history |
| `/health` | Health check for Render |
| `/api/state` | Full council snapshot (includes live `accuracy`) |
| `/api/accuracy` | Chair hit-rate: correct / total directional calls + % |
| `/api/history` | Recent logged signals |
| Canvas Round Table | Cyberpunk Art + Dashboard modes (polls `/api/state`) |
...
| Callsign | Role | Internal key | Specialty |
| --- | --- | --- | --- |
| **WICK** | Pattern Seer | `candle` | candle patterns, S/R |
| **PULSE** | Flow Reader | `volume` | spikes / confirmation |
| **DRIFT** | Trend Scout | `momentum` | RSI / MACD-ish |
| **TAPE** | Book Walker | `orderflow` | Kalshi mid + taker flow |
| **CARRY** | Rate Oracle | `funding` | perp funding / OI |
| **ORBIT** | Regime Watch | `regime` | session / vol aggressiveness |
| **WARDEN** | System Guard | `guardian` | feed health |
| **CHAIR** | The Gavel | `leader` | confluence + final call |
...
See **[DEPLOY_RENDER.md](DEPLOY_RENDER.md)** for the full max setup:

1. Push this repo to GitHub
2. Render → **Blueprint** → select repo (`render.yaml`)
3. Uses **Web Service + 2 GB Disk + Huddle Cron + Keepalive Cron**
